...
syn keyword hoi4Keyword focus shared_focus focus_tree country_event global defined_text guitypes 
syn keyword hoi4Scope every_state every_owned_state random_owned_state ROOT THIS PREV FROM overlord faction_leader controller capital_scope event_target var if else_if else OR NOT AND random_list containerWindowType
syn keyword hoi4Param id icon ai_will_do ai_chance x y value trigger default factor modifier add subtract clamp min max multiply tag original_tag cost log option title desc completion_reward continuous_focus_position country search_filters allow_branch mutually_exclusive prerequisite limit ideology relative_position_id available select_effect name folder days bonus cost_reduction uses category bypass picture allowed allowed_civil_war allowed targeted_modifier removal_cost days_remove visible complete_effect remove_effect tooltip fire_only_once keep_completed tree is_good priority visible_when_empty scripted_gui dynamic localization_key text target immediate is_triggered_only size iconType spriteType
