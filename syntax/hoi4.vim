...
syn keyword hoi4Keyword focus shared_focus focus_tree country_event
syn keyword hoi4Scope every_state every_owned_state random_owned_state ROOT THIS PREV FROM overlord faction_leader controller capital_scope event_target var if else_if else OR NOT AND
syn keyword hoi4Param id icon ai_will_do ai_chance x y value trigger default factor modifier add tag cost log option title desc completion_reward continuous_focus_position country search_filters allow_branch mutually_exclusive prerequisite limit ideology relative_position_id available select_effect name folder days bonus cost_reduction uses category bypass
syn keyword hoi4Bool yes no
syn match hoi4Variable "\v\@[a-zA-Z_0-9]+"
syn match hoi4Operator "\v\="
syn match hoi4Comment "\v#.*$"
syn match hoi4Tag "\v<[A-Z]{3}>"
